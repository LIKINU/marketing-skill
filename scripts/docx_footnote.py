#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""docx_footnote.py — 给 python-docx 产出的 .docx 加**真正的 Word 脚注**
...
# 占位符：用私用区字符包住，正常文本里不可能出现，且不会被 strip_inline 动到
PH_L = "\ue000FN"
PH_R = "FN\ue001"
...
def extract_definitions(md_text):
    """从 Markdown 里抽走 `[^label]: 出处` 定义行。

    回传 (去掉定义行的 markdown, {label: 出处文本})。
    """
    defs = {}
    keep = []
...
# ─────────────────────────────────────────────────────────────
# 渲染侧：把一段文字拆成「普通 run / 粗体 run / 脚注 run 占位」
# ─────────────────────────────────────────────────────────────

class FootnoteState:
    """渲染过程中累积「标签 → 序号」，序号即 Word 脚注 id。"""

    def __init__(self, defs=None):
        self.defs = defs or {}
        self.order = []          # [label, ...] 按正文出现顺序
        self._ids = {}

    def fid(self, label):
        if label not in self._ids:
            self.order.append(label)
            self._ids[label] = len(self.order)      # 1-based；0 留给 continuationSeparator
...
def install_footnotes(docx_path, items, verbose=True):
    """把 items=[(fid, text), ...] 装进 docx_path（就地改写）。回传替换掉的位置数。"""
    if not items:
        return 0
    tmp = docx_path + ".tmp"
    replaced = 0
...
def inspect(docx_path):
    """回传 {has_part, has_ct, has_rel, refs, texts, placeholder_left}"""
...
def main():
    ap = argparse.ArgumentParser(description="查看／校验 .docx 里的 Word 脚注装配")
    ap.add_argument("docx")
    ap.add_argument("--list", action="store_true", help="列出每条脚注")
    ap.add_argument("--verify", action="store_true", help="校验 OOXML 装配完整性")
    a = ap.parse_args()
    if not os.path.exists(a.docx):
        print(f"❌ 找不到 {a.docx}")
        sys.exit(2)
    r = inspect(a.docx)
    print(f"文件：{os.path.basename(a.docx)}")
    print(f"  footnotes.xml part ：{'✅' if r['has_part'] else '❌'}")
    print(f"  Content_Types 声明 ：{'✅' if r['has_ct'] else '❌'}")
    print(f"  document.rels 关系 ：{'✅' if r['has_rel'] else '❌'}")
    print(f"  正文引用数         ：{r['refs']}")
    print(f"  残留占位符         ：{r['placeholder_left']}"
          + ("　⚠️ 有未替换的占位符＝装配没走完" if r['placeholder_left'] else ""))
    if a.list:
