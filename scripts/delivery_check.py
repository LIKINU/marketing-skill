...
# ── 占位符／未完成标记 ──
#   分两级，理由：2026-09-17 实测，「占位」「待补」在营销文稿里是**正常词**
#   （「被头号对手长期占位的品类大词」「心智占位」「一手结论留空待补，执行后回填」），
#   一律当硬错误会到处误报 —— 误报多了，人就开始忽略这张表。
#   硬错误只留「模板占位符」这种一眼是机器的记号；散文词降为警告。
PLACEHOLDER_HARD = ["【填】", "{FILL}", "（填）", "TODO", "TBD", "XXX", "???",
                    "【队长姓名】", "【团队名称】", "【项目名称】",
                    "【团队名】", "（未给出处：", "（未给出处："]
PLACEHOLDER_WARN = ["待补", "待填", "待确认", "待确认", "待核实", "待核实"]

# ── 封面「不该出现」的额外字段（官方通常要求封面只保留极少数项）──
COVER_FORBIDDEN = ["学号", "学号", "指导教师", "指导老师", "指导教师", "姓名：",
                   "电话", "手机", "手机", "邮箱", "邮箱", "@", "赛道", "赛道",
                   "专业", "专业", "班级", "班级"]
# 单独成段的日期／时间（官方若写「仅保留项目名称＋团队名称」，日期也算多余）
COVER_DATE_RE = re.compile(r"^[0-9]{4}\s*[-年./]\s*[0-9]{1,2}\s*([-月./]\s*[0-9]{1,2}\s*日?)?$")


def _is_intermediate(fn):
    """中间产物／备份不参与「文件名规范」校验 —— 它们本来就不该提交，
    但在工作目录里必然存在，混进清单只会把真问题淹掉。"""
...
def check_cover(paras, allow):
    """封面＝ .docx 开头、目录之前的少量段落。这里取前 6 个非空段落做判断。"""
    head = paras[:6]
    facts = []
    for p in head:
...
ANON_WORDS = ["指导教师", "指导老师", "指导教师", "导师：", "导师：", "队长：", "队长：",
              "组员：", "组员：", "组别：", "学号", "学号"]
ANON_SAFE = ["产学研", "产学研", "对标", "对标", "参考", "参考", "案例", "承办", "承办",
             "主办", "主办", "合作院校", "白皮书", "白皮书"]
...
def anon_scan(text):
    ...
    for ln in text.split("\n"):
        ...
        if not safe:
            for m in re.finditer(r"(?:大学|大学|学院|学院|高等专科)", ln):
                facts.append(f"正文出现院校名（非对标语境）：{ln.strip()[:40]}")
                break
        if "学号" in ln or "学号" in ln:
...
def check_declaration(path):
    ...
    if "AI" in os.path.basename(path) or "人工智能" in os.path.basename(path):
        ...
        has_scope = bool(re.search(r"使用(的)?(范围|环节|环节|情况|情况)|用于|用于", txt))
        if not has_tool:
...
def main():
    ...
    ap.add_argument("--manifest", default="", help="附件核对表 JSON（--init 可生成）")
    ap.add_argument("--rules", default="", help="《任务规则表》JSON（其中若有必交附件／封面要求则采用）")
    ap.add_argument("--name-pattern", default="", help="文件名规范（正则，可含命名组）")
    ap.add_argument("--cover-allow", default="", help="封面允许的要素，逗号分隔（如 项目名称,团队名称）")
    ap.add_argument("--init", action="store_true", help="生成可勾选的附件核对表模板")
    ap.add_argument("--strict", action="store_true", help="有未通过项时以退出码 1 结束（默认也返回 1）")
    a = ap.parse_args()

    if not os.path.isdir(a.dir):
        print(f"{NG} 找不到目录：{a.dir}")
        sys.exit(2)
    ...
    if a.init:
        if os.path.exists(mpath):
            print(f"{WARN} 已存在，未覆盖：{mpath}")
        else:
            open(mpath, "w", encoding="utf-8").write(
                json.dumps(man, ensure_ascii=False, indent=2))
            print(f"{OK} 已生成可勾选的附件核对表：{mpath}")
            print("   → 交了的把 \"已交\" 改成 true，或直接把文件放进交付目录（脚本按 glob 自动认）")
    ...
    print("=" * 68)
    print(f"交付形态核对 · delivery_check.py　目录：{os.path.basename(os.path.abspath(a.dir))}")
    print(f"  核对表来源：{src}　｜　目录内文件 {len(files)} 个")
    print("=" * 68)
    ...
    if not rows:
        print(f"  {WARN} 核对表里没有附件项（可用 --init 生成模板）")
    for r in rows:
        mark = OK if r["通过"] else (NG if r["必需"] else WARN)
        how = ("已勾选" if r["已勾选"] else
    ...
    if not docxes:
        print(f"  {WARN} 目录里没有 .docx，跳过封面与占位符检查")
    else:
        # ⚠️ 2026-09-17 改（R2 合规视角第 8 条）：原实现只查**最大的那一个 docx** ——
        ...
        for target in docxes:
            ...
            if paras is None:
                print(f"  {WARN} 缺 python-docx，跳过封面检查（pip install python-docx）")
                break
    ...
    if skipped:
        print(f"  （跳过中间产物／备份 {len(skipped)} 个：{'、'.join(skipped[:3])}…）")
    if not finals:
        print(f"  {WARN} 没有可提交的 .docx/.pdf 可查")
    for f in finals:
    ...
    for target in docxes:
        ...
        if hard:
            print(f"  {NG} {target} 残留模板占位符 {len(hard)} 种："
                  + "、".join(f"{k}×{v}" for k, v in hard.items()))
            fail.append(f"{target} 残留模板占位符：{'、'.join(hard)}")
        if soft:
            print(f"  {WARN} {target} 出现 {len(soft)} 种「未完成」字样（可能是主动声明，"
                  f"也可能真没写完，需人工看一眼）："
                  + "、".join(f"{k}×{v}" for k, v in soft.items()))
        if not hard and not soft:
            print(f"  {OK} {target} 未见占位符——稿子是真写完的")

    # 声明件内容校验（R2-15b）
    print("\n【5】声明件内容（不能只认文件名）")
    decl_files = [f for f in files
                  if re.search(r"AI|人工智能|承诺|承诺|授权|授权|自查|合规|合规", f)
                  and f.lower().endswith((".docx", ".md", ".txt"))]
    if not decl_files:
        print(f"  {WARN} 未见声明件（AI 使用说明／原创承诺／合规自查）—— 官方要求单独提交")
    for f in decl_files:
    ...
    print("\n" + "=" * 68)
    if fail:
        print(f"{NG} 未通过 {len(fail)} 项：")
        for x in fail:
            print(f"   · {x}")
        print("\n→ 这些是**官方硬项**，不是建议；补齐后重跑本脚本。")
        sys.exit(1)
    print(f"{OK} 交付形态全部通过。")
    sys.exit(0)
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n{NG} delivery_check 执行出错：{type(e).__name__}: {e}")
        print("→ 依协议 8：修正后重跑；环境问题就降级为人工逐项核对附件清单。")
        sys.exit(2)
