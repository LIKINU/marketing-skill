...
_M03_RE = {}
# 知識庫裡標了「不得引用／待核實」的模型碼（由 build 時掃描 03 手冊生成）
_M03_BANNED = set()


def _load_m03_banned():
    """掃 03 手冊，把標「不得引用／待核實」的模型碼收進黑名單。"""
    try:
...
def model_brief(code, nsteps=3):
    ...
    # ⚠️ 2026-09-17（連鎖加盟視角第 7 條，實測）：知識庫裡有標「⛔待核實 · 不得引用」的模型，
    #   而 card_line_public 對「未核實」有過濾、model_explain **沒有** —— 同一條規則兩處不一致，
    #   禁用內容會從模型這條路徑進交付稿。
...
def build_skeleton(rules, plays, kmap, tier, cardpoints, scene=""):
    ...
    positioning = (
        ...
        # 2026-09-17（平台算法視角第 6 條）：GUIDE 要求本節是「5–12 行的表」，
        #   而 composer 給的是裸 `{FILL}` —— 骨架與指引漂移，檢查必然落空。
    ...
    budget = (
        f"## 七 · 预算明细\n"
        # 2026-09-17（連鎖加盟視角第 3 條）：要門店出錢出力卻無分攤、核銷與罰則 ——
        #   門店對掏錢的活動必然軟抵抗。
        ...
        # 2026-09-17（平台算法視角第 3 條）：投流在整條流水線裡沒有落點 ——
        #   路由表把「投流虧錢」導向診斷章，客戶拿不到止損動作。
    ...
    exec_ = (
        ...
        f"## 八 · 执行与风控\n"
        f"### 8.1 行动清单\n"
        f"| 行动 | **执行主体（总部／区域／门店／加盟商）** | 负责方（岗位到人） | 替补人 | 截止 | 预算 | 验收标准 | 验收人 | 决策权限（谁能批／超出找谁） | 对执行人的好处 |\n"
        f"|---|---|---|---|---|---|---|---|---|---|\n"
        + "".join(f"| {FILL} | {FILL} | {FILL} | {FILL} | {FILL} | {FILL} | {FILL} | {FILL} | {FILL} | {FILL} |\n"
                  for _ in range(10)) + "\n"
        ...
        # 2026-09-17（法務版權視角第 3／4／5／6／8 條）：全庫「授權／版權」命中約 30 處，
        #   而**沒有一處落在生成稿的必需字段裡** —— 達人塌房、素材侵權、肖像權、UGC 超範圍使用
        #   全都無從自證。這一節是「出事了拿得出東西」的那一節。
        f"### 8.9 外部合作合同要点（**达人／拍摄／场地／物料**）\n"
        f"| 对象 | 排他期 | 内容归属与二次分发权 | 发布后不得删稿期限 | 数据造假与违约金 |\n"
        f"|---|---|---|---|---|\n"
        + "".join(f"| {FILL} | {FILL} | {FILL} | {FILL} | {FILL} |\n" for _ in range(3)) + "\n"
...
SCENE_SECTIONS = {
    ...
    "标准": (
        ...
        # 2026-09-17（平台算法視角第 1／4／7 條）：
        #   ① 算法靠前 5–10 條內容給賬號打人群標籤，而方案只排「發什麼」，不排「怎麼餵標籤」；
        #   ② 達人投放只剩「誰來說」一格，缺分層／佔比／防刷；
        #   ③ 內容產能與素材複用無字段 —— 算法要持續更新，斷更掉權重。
        ...
        f"不用个人号存客户资产：{FILL}\n\n"
        # 2026-09-17（連鎖加盟視角第 1／4 條）：標準檔（最常服務連鎖品牌）原本只鎖價格，
        #   物料／VI／話術零字 —— 門店不知哪些不能改，只能自行發揮；
        #   而且**沒人查門店做沒做**。
        ...
        + "".join(f"| {FILL} | {FILL} |\n" for _ in range(3)) + "\n"
        "### 11.4.1 稽核表（**没人查＝没人做**）\n"
        "| 检查项 | 频率 | 抽查比例 | 合格线 | 不合格处置（整改／扣保证金／停供物料） |\n"
        "|---|---|---|---|---|\n"
        + "".join(f"| {FILL} | {FILL} | {FILL} | {FILL} | {FILL} |\n" for _ in range(3)) + "\n"
        "### 11.4.2 培训与物料下发\n"
        "| 对象 | 课时 | 形式 | 考核合格线 | 物料下发方式 | 到店截止日 | 签收人 |\n"
        "|---|---|---|---|---|---|---|\n"
        + f"| 店长／店员 | {FILL} | {FILL} | {FILL} | 总部直发／区域仓／门店自印 | {FILL} | {FILL} |\n\n"
        ...
        # 2026-09-17（R6 連鎖加盟第 5 條）：只寫「對執行人的好處」不夠 ——
        #   加盟商是**獨立法人**，要算他自己的賬，否則他沒理由配合。
        f"### 14.1 对门店／加盟商的账（**他不配合是因为你没算给他看**）\n"
        f"| 单店投入（元） | 月均增量毛利（元） | 回本月数 | LTV/CAC |\n|---|---|---|---|\n"
        f"| {FILL} | {FILL} | {FILL} | {FILL} |\n"
        f"> 回本月数 > 6 个月 → 加盟商大概率軟抵抗，需要總部補貼或分批投入。\n\n"
        # 2026-09-17（R5 私域第 7 條）：沒有交易載體與標籤口徑，複購券發不對人，方案停在動作層。
        f"## 十五 · 工具与数据底座\n"
