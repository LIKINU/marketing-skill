...
MANUAL = {
    ...
    "14": ("商业模式模式库：**108 条通用模式 ＋ 14 套公开体系**，每条含「本质／钱从哪来／"
           "什么时候用／怎么落地／坑／示例」六段（**二次整理的抽象结构，不署人名与书目**）",
           "**要判断「客户的商业模式属于哪一类、他的钱从哪来」时**"
           "（写策略／定价／第二曲线前）；⛔ **不是营销打法，不要拿它替代 00-打法库**",
           "该客户的钱从哪来、往哪儿加杠杆、这一类模式最常见的坑"),
}
