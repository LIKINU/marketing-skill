...
BANNED_HARD = [
    ...
    "脱敏", "激素", "排毒",
    # ⚠️ 2026-09-19（C 批第五批 · 体系6 #5「未成年人／母婴保护」）：母婴与儿童食品的
    #   「替代母乳／促进长高／变聪明」是**虚假宣传高发区**（罚款级），原先一个都没进表。
    #   ⚠️ 「益智」**故意不收** —— 它是玩具品类的常规功能描述（积木／拼图），
    #      裸禁会把正常玩具文案全拦下；只在食品语境违规，而**这个判据分不清语境**。
    "替代母乳", "促进长高", "变聪明", "长高个", "比母乳",
]
...
BANNED_HARD_RE = [
    ...
    r"独家(?:配方|首发|代理|版权)",
    # ⚠️ 2026-09-19（C 批第五批 · 体系6 #10「竞品比较合规边界」）：**贬低同行**是《广告法》第 13 条
    #   明文禁止的，但**不能裸禁「秒杀」** —— 它是合法的促销机制（限时秒杀）。
    #   → 只拦**带比较对象**的贬低式用法：`碾压/吊打/完爆/秒杀 + 竞品/同行/对手/友商/其他品牌`。
    r"(?:碾压|吊打|完爆|秒杀|吊锤)(?:竞品|同行|对手|友商|其他品牌|别家|别人家)",
]
...
def main():
    ...
    # 「是不是完整版」的判据。
    # ⚠️ 2026-09-19 修：原来要求字面出现「现状分析」—— 而**真人写的完整方案用的是「一 · 诊断」**
    ...
    # 32) C 批第五批：补 8 条「一字都没落过」的维度（2026-09-19）
    #     来源：六体系逐行核对里**真未落**的那 8 条（不是判据空转，是**位置都没有**）。
    #     ⚠️ 与 25/26/28/29/30 同理：判「结构在不在」（表头/字段）＋「内容有没有」交给【31】。
    def _sec32(*keys):
        return _sec_any(*keys)

    _C32 = [
        # (标签, 章节关键词, 表头/正文必须出现的碎片, 说明)
        ("投放账户与冷启动", "投流计划", ["账户结构", "学习期", "关停线"],
         "**账户结构与冷启动纪律决定投手第一天怎么开局**：几个账户、计划怎么分层命名、"
         "学习期几天不调价、花多少还没转化就必须关。缺了这三列，方案交到投手手里没法执行。"),
        ("反向激励检查", "反向激励", ["做歪", "错误行为", "制衡"],
         "**考核什么，就会得到什么的变体。** 只写正向激励＝只考虑了我们想要什么；"
         "这一节要写「这个打法会被怎么钻空子」以及**制衡手段**（第二指标／抽查／惩罚）。"),
        ("未成年人·母婴保护", "未成年人", ["监护人", "禁语"],
         "**涉及校园／儿童／母婴就不能空这一节**：监护人**书面／单独同意**（《个保法》第 31 条）、"
         "未成年人形象使用边界、**母婴食品禁语**（替代母乳／促进长高／变聪明）、校园渠道红线。"),
        ("抽奖与导流合规", "抽奖", ["开奖", "上限", "导流"],
         "**规则要写死，导流要分平台**：参与条件／开奖时间／开奖方式／公示在哪、奖项价值上限、"
         "中奖名单留存；⛔ 不要写一句「全平台同步导流」—— 小红书禁站外导流、抖音限制引导私域、"
         "微信禁诱导分享，必须**按平台分别写可与不可**。"),
        ("竞品比较合规边界", "竞品比较", ["来源", "贬低"],
         "**比较可以，贬低不行**（《广告法》第 13 条）：参数／价格／规格的对比**须附可查来源**，"
         "禁用碾压／吊打／秒杀／完爆这类贬低词，比不过的地方不写或改讲场景。"),
        ("小红书笔记形态", "小红书", ["封面", "标题公式", "标签", "配比"],
         "**小红书是搜索型社区**：标题与标签要按「用户会搜什么」写。四类笔记（痛点／攻略／开箱／对比）"
         "选一、封面主标、标题公式、**标签 ≥5**、**三档配比（干货:种草:转化）** —— "
         "5.1 里「覆盖一行」≠ 写出这家的原生形态。"),
        ("视频号原生形态", "视频号", ["转发理由", "承接", "朋友圈"],
         "**视频号是社交推荐**：完播率不是第一位，「值得被熟人看到」才是。"
         "一句话转发理由／公众号·企微承接／朋友圈分发路径／与抖音那条差在哪。"
         "⛔ 直接把抖音那条搬过来＝没有视频号。"),
        ("责任与保险", "责任与保险", ["谁赔", "上限", "投保"],
         "**出了事谁赔、赔到哪、投保没有**：现场活动（搭台／试吃／亲子／户外）是人身意外高发场景；"
         "「谁赔」要写到主体，「赔到哪」要写**上限**，并写清险种与保额。"
         "只写「按合同约定」等于没写。"),
    ]
    _miss32 = []
    for _lb, _kw, _need, _why in _C32:
        _sec = _sec32(_kw)
        if not _sec:
            _miss32.append(f"{_lb}（整节缺失）")
            continue
        # ⚠️ 2026-09-19 **本关初版就犯了自己一直在修的病**：拿「整节文本」去查关键词 ——
        #   而 composer 的**说明文字里也写着这些词**（例：7.4 的注释写「「学习期规则」与
        #   「冷启动关停线」两列必须带数字」；8.9.2 的注释写「「谁赔」要写到主体」）。
        #   双向测试当场抓到：**删掉整列/整行之后判据照样 ✅**。
        #   → 只认**表头行 ＋ 每行第一格（字段名）**：那是「只有填的人才会写出来的位置」。
        #   ⚠️ 注意 `_tbl()` 回的是 **(表头 cells, [每行 cells])** —— `_rows32` 的每一项
        #      **已经是 cell 列表**，不要再套 `_cells()`（写了会 `AttributeError: list has no strip`）。
        _h, _rows32 = _tbl(_sec)
        _blob = " ".join(_h)
        for _r in _rows32:
            _blob += " " + (_r[0] if _r else "")
        _lacks = [k for k in _need if k not in _blob]
        if _lacks:
            _miss32.append(f"{_lb}（缺 {'／'.join(_lacks)}）")
    if not quiet:
        print(f"  {OK if not _miss32 else NG} 第五批 8 项："
              f"{'8/8 落点齐' if not _miss32 else '缺 ' + '；'.join(_miss32)}")
    if _miss32:
        _why = "；".join(f"{lb}：{w}" for lb, _k, _n, w in _C32
                        if any(lb in m for m in _miss32))
        _hard_if_full(
            f"以下工序维度**没有落点**：{'；'.join(_miss32)} —— 逐条要求：{_why}")
