...
BANNED_HARD = [
    ...
    # 私域專屬紅線（2026-09-17 R5 私域視角第 8 條）：誘導分享一次判罰就能封鏈，
    # 整條私域路徑斷掉 —— 與廣告法同級，故進硬錯誤。
    "诱导分享", "誘導分享", "助力", "集赞", "集讚", "砍价免费拿", "砍價免費拿",
    "分享才能", "不转不是", "转发才能", "个人号收款", "個人號收款",
    # 美妝常見違規宣稱（R2 補）
...
def main():
    ...
    # ── 【17】承接與危機（2026-09-17 隨 R5 一起加：私域／客服輿情兩視角的共同要求）
    #    共同病灶：方案只寫「做什麼」，不寫「出事時誰接」。
    if not quiet:
        print("\n【17】承接與危機（私域閉環／客服／超賣退款／輿情發聲人）")

    # 17a 私域四環是否成鏈（加微→首單→復購→轉介紹）
    if re.search(r"加微钩子|加微鉤子|私域", body):
        _chain = {k: (k in body) for k in ("加微", "首单", "复购", "转介绍")}
        _miss = [k for k, v in _chain.items() if not v]
        if not quiet:
            print(f"  {OK if not _miss else WARN} 私域鏈路四環（加微→首單→複購→轉介紹）："
                  f"缺 {len(_miss)} 環" + (f"：{'、'.join(_miss)}" if _miss else ""))
        if len(_miss) >= 2:
            _hard_if_full(f"私域鏈路缺 {len(_miss)} 環（{'、'.join(_miss)}）—— "
                          f"鏈頭加微與鏈尾轉介紹最容易漏；缺兩環以上不成鏈。")
        # 24 小時起點（起點必須是「加好友後」，不是「購買後」）
        if "24 小时" in body or "24 小時" in body:
            if not re.search(r"(加好友|加微)[^\n]{0,20}24\s*小时|24\s*小时[^\n]{0,20}(加好友|加微)", body):
                warnings.append("私域有 24 小時動作，但沒寫清「起點是加好友後」——"
                                "起點寫成「購買後」，新好友 24 小時就無人管。")

    # 17b 客戶側超賣與退款
    if re.search(r"库存|庫存|超卖|超賣", body) or _is_full_plan:
    ...
    # 17c 輿情：發聲人是否唯一且寫了時限
    if re.search(r"舆情|輿情|对外发声|對外發聲", body):
        _has_delay = bool(re.search(r"\d+\s*小时|\d+\s*小時|当天|當天", body))
        _has_spokes = bool(re.search(r"发声人|發聲人|对外口径|對外口徑|统一口径|統一」?口徑", body))
        if not quiet:
            print(f"  {OK if (_has_delay and _has_spokes) else NG} 舆情：回應時限 "
                  f"{'有' if _has_delay else '缺'}、發聲人／對外口徑 {'有' if _has_spokes else '缺'}")
        if not (_has_delay and _has_spokes):
            hard_errors.append("舆情预案不完整 —— 必须写「几小时内回应」与「谁是唯一发声人」。"
                               "没写的后果：危机时一线各自回话，口径打架。")

    # ── 【18】版權與授權（2026-09-17 隨 R6 法務視角加）
    #    為什麼單獨一關：全庫「授權／版權」命中約 30 處，
    #    而**沒有一處落在生成稿的必需字段裡** —— 出事時甚麼都拿不出來。
    if not quiet:
        print("\n【18】版权与授权（音乐／字体／肖像／UGC／非遗专利）")

    # 18a 音樂：不得用熱門歌
    if re.search(r"BGM|背景音乐|背景音樂|配乐|配樂|音效", body):
        if not re.search(r"免费商用|免費商用|曲庫|曲库|自行录制|自行錄製|已授权|已授權", body):
            hard_errors.append(
                "方案提到 BGM／音效，卻沒寫明**来源** —— 必须写「平台免费商用曲库／自行录制」。"
                "短视频 BGM 是音著协／唱片公司索赔最高频的场景。")
    # 18b 字體授權
    if re.search(r"字体|字體", body):
        if not re.search(r"授权类型|授權類型|系统自带|系統自帶|已购|已購|免费商用|免費商用", body):
            warnings.append("方案提到字体，却未写授权类型（系统自带／已购／免费商用）——"
                            "方正／汉仪批量维权是营销物料最常见的索赔函来源。")
    # 18c 肖像權
    if re.search(r"模特|素人|达人出镜|達人出鏡|拍摄|拍攝", body):
        if "肖像" not in body and "授权书" not in body and "授權書" not in body:
            hard_errors.append("方案涉及出镜拍摄（模特／素人／达人），但没写**肖像／声音授权书** —— "
                               "「免单换拍摄」不等于肖像授权。")
    # 18d 非遺／專利／認證須附證明文件
    if re.search(r"非遗|非遺|专利|專利|认证|認證", body):
        if not re.search(r"证书编号|證書編號|证明文件|證明文件|文件编号|文件編號|ZL\d|有效期", body):
            _hard_if_full("方案宣称「非遗／专利／认证」却**未附证明文件编号与有效期** —— "
                          "这是虚假宣传被举报的高发入口。")
    # 18e UGC 授權
    if re.search(r"UGC|用户投稿|用戶投稿|征集|徵集|打卡征集", body):
        if not re.search(r"授权|授權|同意品牌|商用范围|商用範圍", body):
