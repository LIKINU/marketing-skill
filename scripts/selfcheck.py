...
# 八篇結構（關鍵詞寬鬆匹配，命中任一即可）
SECTIONS = [
    ("執行摘要", ["執行摘要", "执行摘要", "TL;DR", "核心結論", "核心结论"]),
    ("一 · 現狀分析", ["現狀分析", "现状分析", "生意現狀", "生意现状", "問題診斷", "问题诊断"]),
    ("二 · 策略", ["策略", "方案成立的前提", "節奏排期", "节奏排期", "貨盤", "货盘"]),
    ("三 · 定位與口徑", ["定位", "禁用詞", "禁用词", "口徑", "口径", "差異化支點", "差异化支点"]),
    ("四 · 觸達與渠道", ["觸達", "触达", "渠道", "投放", "傳播路徑", "传播路径"]),
    ("五 · 落地文案與物料", ["落地文案", "文案", "物料", "話術", "话术"]),
    ("六 · KPI 與追蹤", ["KPI", "追蹤", "追踪", "指標", "指标", "監測", "监测"]),
    ("七 · 預算明細", ["預算明細", "预算明细", "預算表", "预算表", "盈虧線", "盈亏线"]),
    ("八 · 執行與風控", ["執行與風控", "执行与风控", "行動清單", "行动清单", "風險", "风险", "關鍵假設", "关键假设"]),
]

# 內部過程文檔關鍵詞 —— 硬錯誤（明顯是「怎麼幹活」的文檔名，不該出現在給客戶的稿裡）
INTERNAL_LEAK_HARD = [
    "事實底稿", "事实底稿", "多 Agent", "多Agent", "Agent 分工", "Agent分工",
    "主理人裁決", "主理人裁决", "內部備註", "内部备注",
]
# 軟警告 —— 這些詞在「交付自檢單」裡會合法出現（如「門禁 13 項已問全」），只提示人工確認
# 「裁決記錄」放這裡而非硬錯誤：12 項自檢單第 12 條明文要求「裁決記錄與產出索引」，
# 若判硬錯誤，等於自檢單要求寫、又被自檢單判違規（死結），故降為警告。
INTERNAL_LEAK_SOFT = ["門禁", "门禁", "流程狀態", "流程状态", "裁決記錄", "裁决记录"]
...
BANNED_DEFAULT = [
    ...
    "藥到病除", "药到病除", "特效",
    # 金融
    "保本", "保收益", "穩賺", "稳赚", "零風險", "零风险", "穩賺不賠", "稳赚不赔",
]
...
def main():
    ...
    if not quiet:
        print("\n【2】交付自檢單")
        print(f"  {'✅' if has_checklist else NG} 出現「自檢單」字樣：{has_checklist}")
        print(f"  {'✅' if box_count >= 12 else WARN} ✅/❌ 標記數量：{box_count}（建議 ≥12）")
    if not has_checklist:
        hard_errors.append("未輸出 12 項《交付自檢單》")
    elif box_count < 12:
