...
# ─────────────────────────────────────────────────────────────
ASSIGN = {
    # ── §1 認知類（2026-09-17 補：這 8 條原本是舊的「描述式」案例行，也一律重寫）
    "1.1": [("蜜雪冰城", "01"), ("旺旺", "02")],
    "1.2": [("王老吉", "02"), ("東鵬特飲", "02"), ("安踏", "04")],
    "1.3": [("元氣森林", "02"), ("小米", "05")],
    "1.4": [("農夫山泉", "02"), ("三頓半", "02")],
    "1.5": [("蜜雪冰城", "01"), ("老鄉雞", "01")],
    "1.6": [("農夫山泉", "02"), ("羅森", "14")],
...
import composer as _C          # 用**同一个解析器**，保證「引用行」與「composer 實際取到的卡」一致


def cards_in(prefix):
    """→ [(卡標題, 檔名)]，按檔內出現順序（＝清單 ∪ 深度卡）"""
    out = []
...
def resolve(kw, prefix):
    ...
    ⚠️ 兩次修正（2026-09-17）：
      ① 第一版用「標題最短」當判定，會選中**副卡**——實測把 王老吉 選成
         「王老吉（禮品場景）」、蜜雪冰城 選成「蜜雪冰城（動畫／短劇）」。
      ② 第二版改「檔內首次出現」，但用自己的 `### 3.N` 解析 → 與 composer 的
         「案例清單 ∪ 深度卡」解析不一致（實測 §2.12 引用 保險經紀人 IP，
         composer 卻取不到，因為那張卡只在深度拆解裡）。
      → 現在**直接複用 `composer.parse_cards`**：引用行寫什麼，composer 就取得到什麼。
    """
