...
--------------------------------------------------------------------------
自測記錄（2026-09-14）
測試樣本：$HOME/Desktop/Marketing-skill/example-山霧茶飲開業季方案.md
          （166 行，由 示例交付稿-山霧茶飲開業季方案.docx 還原；該 docx 八篇目錄
          齊全、12 項自檢單全 ✅，是典型「結構對但很空」的空殼稿 ——
          selfcheck.py 可全綠，本腳本應報多項 FAIL。）

實跑：python3 depth_check.py example-山霧茶飲開業季方案.md
...
自測（同一組閾值跑兩個已知樣本，2026-09-14 實測）：
  ...
  python3 depth_check.py campaign-zuel-lawson-2026-08-29.md → 【9】全綠（重複句 0、跨節 0、
...
def check_repeats(lines, text):
    ...
    校準（同一組閾值跑兩個已知樣本，2026-09-14 實測）：
      原版   campaign-zuel-lawson-2026-08-29.md（371 行）→ ①0 句 ②0 對 ③0 處
                                                            ④最高 3 次 → 本維度全綠
